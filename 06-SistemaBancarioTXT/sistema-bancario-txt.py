...
class ContaCorrente(Conta):
    ...
    @classmethod
    def new_account(cls, number: int, client: PessoaFisica, limit=500.0, limit_saques=3):
        return cls(number, client, limit, limit_saques)
...
def log_transactions(function):
    def package(*args, **kwargs):
        ...
        date_time = datetime.now(timezone.utc).strftime("%d-%m-%Y, %H:%M:%S")
        with open(ROOT_PATH / 'logs.txt', 'a', encoding='utf-8') as file:
            file.write(
                f"[{date_time}]"
                f"Função '{function.__name__}' executada com argumentos {args} e {kwargs}."
                f"Retornou {result}\n"
            )
        print(f'{date_time} => {function.__name__.upper()}')
        return result
